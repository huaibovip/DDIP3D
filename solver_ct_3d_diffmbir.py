...
class Diffusion(object):
    ...
    def sample(self):
        ...
        print(
            "Run DDS 2D for CT reconstruction.",
    ...
    def simplified_ddnm_plus(self, model):
        ...
        save_root = Path(
            f"{self.args.save_root}/{config_file}/{self.args.deg}_view{self.args.Nview}/test_slice{self.args.num_test_slice}"
        )
        save_root = (
            save_root
            / f"adapt_{self.adaptation}_mc{self.args.num_mc}"
            / f"lr{self.args.lr}_{self.args.num_steps}"
            / f"n_ADMM{n_ADMM}"
            / f"n_CG{n_CG}"
            / f"rho{self.args.rho}"
            / f"lamb{self.args.lamb}"
        )
        ...
        for fname in fname_list:
            ...
            img = torch.from_numpy(
                np.load(os.path.join(root, fname), allow_pickle=True)
            )
            h, w = img.shape
        ...
        for i, j in tqdm.tqdm(times_pair, total=len(times)):
            ...
            if args.adaptation:
                ...
                t_mod = int((t // 20).item())
                if (self.args.end_t <= t <= self.args.start_t) and (
                    t_mod % self.args.adapt_every_k == 0
                ):
                    ...
                    for _ in range(args.num_steps):
                        ...
                        for _ in range(self.num_accumulation_round):
                            ...
                            bcg = ATy[samp_idx : samp_idx + self.max_gpu_mc, ...].view(
                                self.max_gpu_mc,
                                1,
                                config.data.image_size,
                                config.data.image_size,
                            )
                            y_idx = y[samp_idx : samp_idx + self.max_gpu_mc, ...].view(
                                self.max_gpu_mc, 1, yh, yw
                            )
                            xt = (
                                xs[-1][samp_idx : samp_idx + self.max_gpu_mc, ...]
                                .to(self.device)
                                .view(
                                    self.max_gpu_mc,
                                    1,
                            ...
                            loss = (
                                adapt_loss_fn(A(x0_t), y_idx)
                                / self.num_accumulation_round
                            )
                            loss.backward()
                        optim.step()
        ...
        for name, module in model.named_modules():
            if isinstance(
                module, (LoraInjectedLinear, LoraInjectedConv2d, LoraInjectedConv1d)
            ):
                adapted_lora_weights[name] = {
                    pname: p.data.clone().to("cpu")
